...
@Component({
    ...
    template: `
		<ng-container>
			...
			<a *ngIf="(!item.routerLink || item.children) && item.visible !== false" [attr.href]="item.url" (click)="itemClick($event)"
			   [ngClass]="item.class" [attr.target]="item.target" tabindex="0" pRipple>
				<i [ngClass]="item.icon" class="layout-menuitem-icon"></i>
				<span class="layout-menuitem-text">{{item.label}}</span>
				<i class="pi pi-fw pi-angle-down layout-submenu-toggler" *ngIf="item.children"></i>
			</a>
			<a *ngIf="(item.routerLink && !item.children) && item.visible !== false" (click)="itemClick($event)" [ngClass]="item.class"
